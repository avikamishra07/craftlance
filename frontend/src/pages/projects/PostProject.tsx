...
interface WizardData {
  title: string; description: string; project_type: ProjectType
  required_skills: string[]; budget_min: string; budget_max: string
  deadline: string; status: 'open' | 'draft'
}

const INITIAL: WizardData = {
  title: '', description: '', project_type: 'fixed',
  required_skills: [], budget_min: '', budget_max: '', deadline: '', status: 'open',
}
...
export default function PostProject() {
  ...
  const validateStep = (s: number): boolean => {
    ...
    if (s === 1) {
      if (data.title.trim().length < 10) errs.title = 'Title must be at least 10 characters'
      if (data.description.trim().length < 50) errs.description = 'Description must be at least 50 characters'
    }
    if (s === 2) {
      if (data.required_skills.length === 0) errs.required_skills = 'Add at least one required skill'
      const min = Number(data.budget_min), max = Number(data.budget_max)
      if (!data.budget_min || isNaN(min) || min < 10) errs.budget_min = 'Minimum budget must be at least $10'
      if (!data.budget_max || isNaN(max) || max < min) errs.budget_max = 'Maximum must be ≥ minimum budget'
    }
    setErrors(errs)
  ...
  const addSkill = (skill: string) => {
    const t = skill.trim()
    if (!t || data.required_skills.includes(t) || data.required_skills.length >= 15) return
    set('required_skills', [...data.required_skills, t])
    setSkillInput('')
  }
  const removeSkill = (skill: string) =>
    set('required_skills', data.required_skills.filter((s) => s !== skill))

  const handlePublish = async (publishStatus: 'open' | 'draft') => {
    setIsSubmitting(true); setApiError(null)
    try {
      const payload: CreateProjectPayload = {
        title: data.title.trim(), description: data.description.trim(),
        project_type: data.project_type, required_skills: data.required_skills,
        budget_min: Number(data.budget_min), budget_max: Number(data.budget_max),
        deadline: data.deadline || undefined, status: publishStatus,
      }
      const project = await createProject(payload)
      navigate(`/projects/${project.id}`)
    } catch (err: any) {
      setApiError(err.response?.data?.detail ?? 'Failed to post project. Please try again.')
        ...
        <div className="space-y-5">
          <div>
            ...
            <input type="text" value={data.title} onChange={(e) => set('title', e.target.value)}
              placeholder="e.g. Build a full-stack SaaS dashboard with React + FastAPI"
              className={cn('input-premium w-full', errors.title && 'border-red-500/50')} maxLength={500} />
            <div className="flex justify-between mt-1">
              <FieldError msg={errors.title} />
              <span className="text-xs text-muted-foreground ml-auto">{data.title.length}/500</span>
            </div>
          </div>
          <div>
            ...
            <div className="flex justify-between mt-1">
              ...
              <span className={cn('text-xs ml-auto', data.description.length < 50 ? 'text-muted-foreground' : 'text-green-400')}>{data.description.length} chars</span>
            </div>
          </div>
          <div>
              ...
              {(['fixed', 'hourly'] as ProjectType[]).map((type) => (
                <button key={type} onClick={() => set('project_type', type)}
                  ...
                  <p className="text-xs text-muted-foreground">
                    {type === 'fixed' ? 'Pay a set amount for the entire project' : 'Pay per hour — good for ongoing work'}
                  </p>
                </button>
              ))}
            </div>
        ...
        <div className="space-y-5">
          <div>
            ...
            <div className="flex flex-wrap gap-1.5 mb-3">
              {SKILL_SUGGESTIONS.filter((s) => !data.required_skills.includes(s)).slice(0, 10).map((s) => (
            ...
            <p className="text-xs text-muted-foreground mt-1">{data.required_skills.length}/15 skills</p>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <div className="relative">
                ...
                <input type="number" min={10} value={data.budget_min} onChange={(e) => set('budget_min', e.target.value)}
                  placeholder="500" className={cn('input-premium w-full pl-7', errors.budget_min && 'border-red-500/50')} />
              </div>
            ...
            <div>
              ...
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground text-sm">$</span>
                <input type="number" min={10} value={data.budget_max} onChange={(e) => set('budget_max', e.target.value)}
                  placeholder="2000" className={cn('input-premium w-full pl-7', errors.budget_max && 'border-red-500/50')} />
              </div>
              <FieldError msg={errors.budget_max} />
            </div>
          </div>
          {data.budget_min && data.budget_max && Number(data.budget_max) >= Number(data.budget_min) && (
            <div className="p-3 rounded-xl bg-brand-500/5 border border-brand-500/15 text-sm text-brand-300">
              Budget range: {formatCurrency(Number(data.budget_min))} – {formatCurrency(Number(data.budget_max))}
            </div>
          )}
        </div>
      )
      ...
      case 3: return (
        <div className="space-y-5">
          <div>
            ...
            <input type="date" value={data.deadline} min={new Date().toISOString().split('T')[0]}
              onChange={(e) => set('deadline', e.target.value)} className="input-premium w-full" />
            <p className="text-xs text-muted-foreground mt-1">Optional — leave blank if flexible</p>
          </div>
          <div className="p-4 rounded-xl bg-surface-3 border border-white/[0.05] space-y-2">
            <p className="text-sm font-medium">Hiring tips</p>
            {['Projects with clear deadlines get 40% more quality proposals',
              'Be realistic — rushed deadlines attract inflated bids',
              'You can always negotiate the timeline after hiring'].map((tip) => (
              <p key={tip} className="text-xs text-muted-foreground flex gap-2">
                <CheckCircle className="h-3.5 w-3.5 text-green-400 shrink-0 mt-0.5" /> {tip}
              </p>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="glass rounded-2xl border border-white/[0.06] p-5 space-y-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground leading-relaxed line-clamp-4">{data.description}</p>
            </div>
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <p className="text-xs text-muted-foreground uppercase tracking-wider mb-1">Budget</p>
                <p className="font-medium">{formatCurrency(Number(data.budget_min))} – {formatCurrency(Number(data.budget_max))}</p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground uppercase tracking-wider mb-1">Type</p>
                <p className="font-medium">{data.project_type === 'fixed' ? 'Fixed Price' : 'Hourly'}</p>
              </div>
              {data.deadline && (
                <div>
                  <p className="text-xs text-muted-foreground uppercase tracking-wider mb-1">Deadline</p>
                  <p className="font-medium">{new Date(data.deadline).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</p>
                </div>
              )}
            </div>
            <div>
          ...
          <div className="grid grid-cols-2 gap-3 pt-2">
            <button disabled={isSubmitting} onClick={() => handlePublish('draft')}
              className="px-4 py-3 rounded-xl text-sm font-medium bg-surface-3 border border-white/[0.06] text-muted-foreground hover:text-foreground hover:bg-surface-4 transition-colors disabled:opacity-50">
              Save as Draft
            </button>
            <button disabled={isSubmitting} onClick={() => handlePublish('open')}
              className="px-4 py-3 rounded-xl text-sm font-medium bg-gradient-brand text-white hover:opacity-90 transition-opacity disabled:opacity-60 flex items-center justify-center gap-2">
              {isSubmitting ? <><Loader2 className="h-4 w-4 animate-spin" /> Publishing…</> : 'Publish Project'}
            </button>
          </div>
        </div>
      )
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8">
      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">Find the perfect freelancer for your next project</p>
      </motion.div>
