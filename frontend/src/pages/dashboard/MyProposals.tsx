...
> = {
  ...
  withdrawn: {
    label: 'Withdrawn',
    icon: XCircle,
    className: 'text-muted-foreground bg-white/5',
  },
}
...
export default function MyProposals() {
  ...
  // Safe pending check
  const hasPending = (myProposals || []).some(
    (p: any) =>
      p.ai_score == null &&
      p.status === 'pending'
  )
    ...
    <div className="max-w-3xl mx-auto py-6 space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">
          {myProposals.length} proposal
          {myProposals.length !== 1 ? 's' : ''}
          {' submitted'}
        </p>
      </div>
